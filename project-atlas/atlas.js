(() => {
  ...
  const cleanupDefinitions = [
    { id: 'builds', title: '1. Build recovery set', paths: 'builds/', status: 'Retained', statusClass: 'keep', description: 'Contains the latest Play Store upload ZIP, matching installable APKS, latest versioned Itch ZIP, and a checksum manifest. Older and duplicate builds were removed.', test: (file) => isPath(file, 'builds') },
    { id: 'agent-temp', title: '2. Temporary agent work', paths: '.codex-tmp/', status: 'Cleaned', statusClass: 'keep', description: 'The redundant worktrees were removed after both branches were pushed. Remaining screenshots, test copies, userdata, and task output were deleted.', test: (file) => isPath(file, '.codex-tmp') },
    { id: 'godot-cache', title: '3. Godot imported texture cache', paths: '.godot/imported/', status: 'Rebuilding automatically', statusClass: 'keep', description: 'The 2.7 GiB cache was removed. Godot immediately began rebuilding current textures from the source assets, so this area will grow again while the project is used.', test: (file) => file[fields.path].startsWith('.godot/imported/') },
    { id: 'generated-media', title: '4. Image-generation batches', paths: 'tmp/imagegen/ · output/imagegen/', status: 'Approved sources retained', statusClass: 'keep', description: 'Superseded batches and untracked output were removed. Four approved Dragon source files referenced by current documentation remain.', test: (file) => /^(tmp|output)\/imagegen\//.test(file[fields.path]) },
    { id: 'android-build', title: '5. Android build folder', paths: 'android/build/', status: 'Tracked template retained', statusClass: 'keep', description: 'Gradle caches and generated packages were removed. The tracked Android build template and configuration remain.', test: (file) => file[fields.path].startsWith('android/build/') }
  ].map((definition) => {
