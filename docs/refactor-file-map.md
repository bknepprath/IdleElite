...
- ActivityDataCatalog global class registration deletion: deleted the unused `class_name ActivityDataCatalog`; `scripts/main.gd` keeps using its explicit local preload/factory unchanged; measured `scripts/main.gd` at 10,829 lines.
- VisualTextureCache global class registration deletion: deleted the unused `class_name VisualTextureCache`; `scripts/main.gd` keeps using its explicit local preload/type/factory unchanged; measured `scripts/main.gd` at 10,829 lines.
- ProfileChatOverlaySurface MobileScrollContainer main-relay deletion: deleted the stale `scripts/main.gd` preload relay, added the owner-local ProfileChatOverlaySurface preload, and retargeted chat overlay scroll construction to `MobileScrollContainer.new()`; measured `scripts/main.gd` at 10,830 lines.
- ActivityCardDepth global class registration deletion: deleted the unused `class_name ActivityCardDepth`; `ThemeStyles` now uses an owner-local preload for its existing typed helper, while existing UI owner preloads stayed unchanged; measured `scripts/main.gd` at 10,831 lines.
- ConvergenceMultiProgressBar global class registration deletion: deleted the unused `class_name ConvergenceMultiProgressBar`; `SkillSwipeActivitySurface` now uses an owner-local preload for its existing typed cast, matching `SkillDetailSurface`; measured `scripts/main.gd` at 10,831 lines.
- HubSurface global class registration deletion: deleted the unused `class_name HubSurface`; `scripts/main.gd` keeps using its explicit local preload/type/factory; measured `scripts/main.gd` at 10,831 lines.
- ShopSurface global class registration deletion: deleted the unused `class_name ShopSurface`; `scripts/main.gd` and boot warmup keep using explicit local preloads; measured `scripts/main.gd` at 10,831 lines.
- CrashReportRuntime global class registration deletion: deleted the unused `class_name CrashReportRuntime`; live callers keep using explicit local preloads of `scripts/diagnostics/crash_report_runtime.gd`; measured `scripts/main.gd` at 10,831 lines.
- ChatState global class registration deletion: deleted the unused global `ChatState` registration; live callers keep using explicit local preloads of `scripts/online/chat_state.gd`; measured `scripts/main.gd` at 10,831 lines.
- Profile/avatar public bridge rename: `ProfileChatOverlaySurface` now exposes `open_profile_overlay()` and `profile_avatar_frame()` for leaderboard presentation; measured `scripts/main.gd` at 10,831 lines.
- LeaderboardProfile global class registration deletion: deleted the unused `class_name LeaderboardProfile`; live callers keep using explicit local preloads of `scripts/leaderboard/profile.gd`; measured `scripts/main.gd` at 10,831 lines.
- PaperButtonStyles global class registration deletion: deleted the unused `class_name PaperButtonStyles`; callers keep using explicit local preloads and static calls; measured `scripts/main.gd` at 10,831 lines.
- OnlineRuntime global class registration deletion: deleted the unused `class_name OnlineRuntime`; `scripts/main.gd` keeps constructing the runtime through its local preload/factory; measured `scripts/main.gd` at 10,831 lines.
- ActivityCardBorder global class registration deletion: deleted the unused `class_name ActivityCardBorder`; real callers keep using owner-local preloads; measured `scripts/main.gd` at 10,831 lines.
- Rounded art helper global class registration deletion: deleted unused `class_name` registrations for `RoundedTextureRect` and `RoundedCornerCropOverlay`; real callers keep using owner-local preloads; measured `scripts/main.gd` at 10,831 lines.
- Rooster Punch-Out stage global class relay deletion: deleted the unused `class_name RoosterPunchOutStage`; real callers keep using owner-local preloads; measured `scripts/main.gd` at 10,831 lines.
- TestStateRuntime class-name main preload relay deletion: deleted the stale `scripts/main.gd` preload; typed vars and construction use the global `class_name TestStateRuntime`; measured `scripts/main.gd` at 10,831 lines.
- Shop rate prompt save bridge ownership: `ShopSurface` owns save/restore methods for `shop_rate_prompt_dismissed`; `SaveRuntime` no longer reads/writes `rate_prompt_dismissed` directly; measured `scripts/main.gd` at 10,832 lines.
- ChatState main preload relay deletion: deleted the stale `scripts/main.gd` preload; OnlineRuntime and ProfileChatOverlaySurface call owner-local `ChatState` directly; measured `scripts/main.gd` at 10,832 lines.
- ActionArtUi main preload relay deletion: deleted the stale `scripts/main.gd` preload; UI/fishing owners already preload `scripts/ui/action_art_ui.gd` directly; measured `scripts/main.gd` at 10,833 lines.
- SkillSwipeActivitySurface gauge constructor host relay deletion: skill-swipe preview header constructs `FishCircle` and `RegenCircle` through owner-local preloads while retaining `scripts/main.gd` consts for remaining users; measured `scripts/main.gd` at 10,834 lines.
- Thieving heist preview host relay deletion: skill-swipe preview construction calls `host._thieving_surface()._build_thieving_heist_card(...)` directly; measured `scripts/main.gd` at 10,834 lines.
- Activity control main preload relay cleanup: `ActivityCardDepth` and `ActivityProgressRail` are directly preloaded by their UI owners; deleted the stale `scripts/main.gd` relays and retargeted checked-in assertions; measured `scripts/main.gd` at 10,834 lines.
- Convergence progress bar ownership relay cleanup: `SkillDetailSurface` now preloads `ConvergenceMultiProgressBar` directly; deleted the host relay property and `scripts/main.gd` preload; measured `scripts/main.gd` at 10,836 lines.
- ActivityDataCatalog accessor deletion: removed `scripts/main.gd::_activity_data_catalog()`; checked-in harnesses read `activity_data_catalog` directly; measured `scripts/main.gd` at 10,844 lines.
- ActivityQueue recovery-full predicate move: `ActivityQueueRuntime` now asks `RecoveryModules.recovery_target_is_full(...)`; measured `scripts/main.gd` at 10,838 lines.
- LeaderboardState presentation relay deletion: removed state-to-presentation display-copy relays; leaderboard page and online row formatting call `LeaderboardPresentation` directly; measured `scripts/main.gd` at 10,844 lines.
- HubSurface HubRuntime state proxy cleanup: deleted `HubSurface` pass-through properties for hub mission cooldown, mission list, and selected module; `HubSurface` now reads/writes those fields through `_hub_runtime()` directly; measured `scripts/main.gd` at 10,858 lines.
- Thieving action-jail state relay cleanup: deleted `scripts/main.gd` action-jail relay properties; callers now use `thieving_state.action_jails` / `last_action_jail_process_unix` directly; measured `scripts/main.gd` at 10,848 lines.
...
- Chat style fold-in: deleted `scripts/chat/styles.gd`; profile/chat overlay owns chat StyleBox helpers and navigation shell gets its unread dot through `ProfileChatOverlaySurface`; measured `scripts/main.gd` at 10,861 lines.
...
- Leaderboard styles owner fold-in: deleted `scripts/leaderboard/styles.gd`; leaderboard page StyleBox factories now live in `scripts/leaderboard/presentation.gd`, and profile/avatar form StyleBox factories now live in `scripts/ui/profile_chat_overlay_surface.gd`; measured `scripts/main.gd` at 10,860 lines.
...
- Step 71 follow-up: app performance runtime and its debug monitor now live together in `scripts/app/performance_runtime.gd`; deleted the one-owner `scripts/app/perf_monitor.gd`; measured `scripts/main.gd` at 10,858 lines after the fold-in.
...
- Step 87 cleanup: leaderboard profile/auth save normalization, restore mutation, and claim-valid gate bodies now live in existing `scripts/leaderboard/profile.gd`; measured `scripts/main.gd` at 35,538 lines after compatibility wrappers.
...
- Current cleanup: leaderboard style pass-through wrappers were deleted from `scripts/main.gd`; leaderboard page styles now live in `scripts/leaderboard/presentation.gd`; measured `scripts/main.gd` at 28,430 lines.
...
- Current cleanup: Berry Prep module button construction folded into `scripts/ui/material_collection_surface.gd`; deleted the one-owner helper; measured `scripts/main.gd` at 11,118 lines.
- Current cleanup: SettingsSurface paper-button presentation now calls `PaperButtonStyles` directly through an owner-local helper; measured `scripts/main.gd` at 10,844 lines.
- Current cleanup: ShopSurface ad stack meter presentation now calls `AdBonus.stack_*` directly; deleted the owner-local stack math relays; measured `scripts/main.gd` at 10,844 lines.
- Current cleanup: AchievementOverlaySurface now preloads `MobileScrollContainer` directly for its home achievements page, achievements modal, and offline summary modal scroll hosts; measured `scripts/main.gd` at 10,844 lines.
- Current cleanup: BuildableModules host-coupled build transaction moved into `scripts/gameplay/action_runtime.gd`; `BuildableModules` is back to build-contract policy/save filtering; measured `scripts/main.gd` at 10,844 lines.
- Current cleanup: achievement milestone builders, target tables, and stamina reward formulas folded into `scripts/achievements/state.gd`; deleted the one-owner milestone helper script; measured `scripts/main.gd` at 10,837 lines.
- Current cleanup: AudioDirector reads active action stamina cost directly from `ActionRuntime` for music intensity; deleted the owner-local relay; measured `scripts/main.gd` at 10,844 lines.
- Current cleanup: SaveRuntime and TestStateRuntime now read/write silver opportunity tip metadata directly on `OnboardingRuntime`; deleted the `scripts/main.gd` save relay properties and retargeted save-normalization probes; measured `scripts/main.gd` at 10,838 lines.
- Current cleanup: CleanProgressBar preload relay deleted from `scripts/main.gd`; remaining annotations/casts use the global `class_name CleanProgressBar`; measured `scripts/main.gd` at 10,837 lines.
...
| --- | ---: | --- |
| `scripts/main.gd` * | 10,831 | Monolithic game controller: save/load wrappers, activity data aliases, skill UI, navigation, fishing, leaderboard, chat, hub, audio, and most orchestration. Primary deletion/refactor target; ActionArtUi, ChatState, CleanProgressBar, ConvergenceMultiProgressBar, and TestStateRuntime no longer need main preload relays. |
...
| `scripts/ui/activity_lock_rig.gd` | 1,141 | Activity lock rig drawing/animation support. |
| `scripts/ui/activity_lock_cluster.gd` | 550 | Activity lock cluster rendering. |
| `scripts/fishing/fluid_strip.gd` | 276 | Fishing fluid strip visual. |
| `scripts/fishing/fluid_strip.gdshader` | 50 | Fishing strip shader. |
| `scripts/fishing/fluid_strip_underlay.gdshader` | 34 | Fishing strip underlay shader. |
...
| `scripts/ui/activity_card_styles.gd` * | 108 | Activity/action-card StyleBox factories plus owner-local page-switch face and prism connector controls. `scripts/main.gd` keeps caches and wrapper names. |
| `scripts/ui/activity_progress_rail.gd` | 532 | Activity progress rail and rail-owned opportunity overlay drawing. |
| `scripts/ui/action_art_ui.gd` * | 613 | Action art image construction, private texture/animation rect classes, shared mask shader, animated art setup, corner badges, border overlay, and headless-safe texture/mask decisions. `scripts/main.gd` supplies texture callbacks and game-specific icon paths. |
...
| `scripts/ui/passive_module_styles.gd` * | 281 | Passive module currency/stat/popup/icon/plank/round/upgrade StyleBox factories plus owner-local serpentine progress bar. Passive/firepit UI surfaces pass current panel, ink, gold, and theme callbacks directly. |
| `scripts/ui/stop_hold_circle.gd` | 22 | Stop-hold progress circle. |
| `scripts/ui/convergence_multi_progress_bar.gd` * | 203 | Convergence progress bar drawing; local preloads own references instead of a global `class_name`. |
...
| `scripts/ui/material_collection_surface.gd` * | 959 | Material collection UI owner: row/module construction, connector positioning, honey/berry module controls, Berry Prep material-module button/hint badge/styles, Berry Mode prep button callback, badge sync, full-screen berry border overlay, material-card style, show/hide layout sync, and material reward flyer/pulse feedback. `scripts/main.gd` keeps direct scene/test compatibility wrappers. |
...
| `scripts/ui/shop_surface.gd` * | 263 | Shop page/ad presentation owner: shop page layout, rewarded-ad offer button, bonus stack light meter nodes, and stack meter sync. Stack duration/count math lives in `scripts/monetization/ad_bonus.gd` and is called directly. |
| `scripts/ui/settings_surface.gd` * | 1,316 | Settings page UI owner: settings page construction, audio volume controls, toggle rows/styles, owner-local paper-button style helper, slider styling/grabber cache, settings control registries, active audio slider drag routing, control sync helpers, reset-confirmation state/logic/feedback, navigation state preservation, and notification settings notice/tween lifecycle. `scripts/main.gd` keeps persisted setting mutations, save calls, audio/dark-mode/gameplay effects, notification permission request, hard-reset data wipe orchestration, paper-button compatibility for other surfaces, and compatibility wrappers. |
...
| `scripts/ui/profile_chat_overlay_surface.gd` * | 1,726 | Profile/chat overlay owner: profile overlay node/state/callbacks, profile save/close/name-submit/account-claim entry, avatar picker state, avatar texture cache, avatar presentation and avatar/name-field StyleBox factories, chat strip construction/styles and visibility, chat overlay shell/rows/status notice/composer/styles, chat row Central-time timestamp formatting, mobile keyboard lift/preview, chat strip input/open-close routing, status copy, draft tracking, submit polling/deferred submit, and send button input. `scripts/main.gd` keeps online transport/profile identity save keys and thin compatibility wrappers. |
...
| `scripts/ui/skill_detail_surface.gd` * | 7,398 | Skill detail surface owner: skill detail rendering, action-card construction helpers, buildable/convergence build overlay widget construction, owner-local ConvergenceMultiProgressBar preload, detail lazy plan/mount/cache helpers, layout transition animation, and swipe-finalized lazy-detail stack replacement/free/visible-card mount batching. `scripts/main.gd` keeps gesture routing, swipe covers, queued swipe navigation, pending finalize processing, and thin compatibility wrappers. |
| `scripts/ui/hub_surface.gd` * | 3,248 | Hub UI surface owner: hub page rendering, module/trophy/decor layout helpers, hotspot hold and drag handling, detail popup and mission board panels/slabs, hub tutorial tip rendering, build smoke/progress overlays, hub mission action-card badge/help popover and icon-pop completion ceremony presentation, and direct `HubRuntime` reads/writes for hub mission cooldown, mission list, and selected module UI state. |
| `scripts/ui/skill_swipe_activity_surface.gd` * | 3,854 | Skill swipe/activity surface owner: activity queue page buttons, empty copy, queue-selection banner, queue-selection card key/toggle helpers, queue number overlays, preview page/card construction, preview state updates, preview prewarm, placeholders, light-preview cards/styles, parked preview pages, preview/global real-card-cache helpers, owner-local convergence progress bar cast, and direct HubSurface mission-badge sync calls for real action cards. `scripts/main.gd` keeps wrappers plus queue storage/runtime bridge, gesture, navigation, cover, and pending detail-finalize orchestration; the temporary dynamic-property bridge was deleted in Step 54. |
...
| `scripts/ui/navigation_shell.gd` * | 4,578 | Navigation shell owner: bottom-nav build/routing/visibility, bottom-nav input hit tests, lock checks/messages, red-X return routing, bottom-nav new-symbol dots and seen-id save bridge, Home/Hub/Shop unlock fade state and locked-message floats, page-switch render-cover pending transition state/release dispatch, module utility row builders/return routing/sort menu builders/scene refs/input/animation/collapse state, skill menu/page state, and pinned/queue pages plus active-shelf and pinned empty-state decor presentation. `scripts/main.gd` keeps gameplay unlock predicates, page functions, physical page-switch cover drawing, sort preference/save policy, pinned module copy building, and retained scene-call compatibility wrappers/properties. |
...
| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/achievements/state.gd` * | 987 | Achievement live-state facade/runtime helpers plus private milestone builders: host-backed milestone context, action medal records, milestone target tables/stamina reward formulas, medal counts/tier summaries, mastery action filtering, skill medal stamina bonus math, global reward bonus and global medal buff policy, neighbor medal buff totals/contributions/lines/rate bonuses, medal accent conversion, visible achievement lists, save-state normalization, completed/new achievement filtering, reward-bonus filtering, and toast seen-id mutation. |
| `scripts/achievements/presentation.gd` * | 170 | Achievement art paths and presentation/style helpers: card, toast queue badge, skill-section StyleBox factories, skill-level target sequence, medal cluster counts/positions, medal texture/silhouette cache helpers, and progress percentages used by home/log UI. |
...
| `scripts/gameplay/buildable_modules.gd` * | 93 | Stateless buildable module policy/save helper: action keys, build contract checks, built-state save/restore filtering, cost dictionaries, labels, XP rewards, affordability checks, and wallet spend. Live build transaction mutation now belongs to `scripts/gameplay/action_runtime.gd`. |
| `scripts/gameplay/recovery_modules.gd` * | 66 | Recovery module policy: recovery contracts, target-skill selection including lowest-stamina targeting, target-full checks, stamina restoration application, and result text. |
| `scripts/gameplay/action_runtime.gd` * | 1,873 | Action lifecycle/runtime owner: live action processing/start/card-tap entry, buildable action transaction mutation, temporary-event ticking, canceled-progress decay, material reward helpers, offline active/convergence simulation, diamond arena predicate, and action-opportunity state machine/window/click/boost/regen helpers. Convergence runtime state/math is reached directly through `host._convergence_runtime()`. `scripts/main.gd` keeps visible rail lookup, floating opportunity feedback, and SFX wrappers. |
...
| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/app/lifecycle_runtime.gd` * | 328 | App lifecycle owner: notification routing, app/runtime background maintenance scheduling, suspend/resume save and repair flow, shutdown cleanup/nulling, headless quit, and shared variant-safe tween cleanup. Shutdown cleanup now releases settings notification state through `SettingsSurface` and profile overlay refs/cache through `ProfileChatOverlaySurface`; `scripts/main.gd` keeps callback/test wrappers. |
| `scripts/app/performance_runtime.gd` * | 406 | App performance owner: desktop/mobile frame caps, mobile battery governor state, adaptive VSync setup, debug-only performance overlay boot policy, battery visual-work predicate, and owner-local runtime performance monitor/debug overlay. |

## Leaderboard Helper Scripts

| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/leaderboard/profile.gd` * | 188 | Leaderboard profile avatar clamping, display-name cleanup, name-key generation/validation, guest-name detection/generation, player-id generation/sanitization, profile/auth save normalization, profile/auth restore mutation, claim-valid gating, auth provider normalization, and refresh-token cleanup. |
| `scripts/leaderboard/state.gd` * | 242 | Leaderboard state/policy owner: category ids/labels, score calculation, submit readiness, cached rows-by-category access, legacy total-level XP scrubbing, and submitted-score/fetch-retry save normalization. Presentation copy/formatting lives in `scripts/leaderboard/presentation.gd`. |
| `scripts/leaderboard/presentation.gd` * | 537 | Leaderboard page UI construction plus display policy: score/rank text, submit status title/detail copy, simple status normalization, empty-state detail copy, and dropdown/player-card/rank-badge StyleBox factories. Scoring/state policy lives in `scripts/leaderboard/state.gd`. |

## Online Runtime Scripts

| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/online/online_runtime.gd` * | 2,078 | Online owner: Firebase config/URL guards, auth, cloud-save status/payload/transport, leaderboard transport, chat transport, profile reference sync, and profile overlay status/focus handoff through `ProfileChatOverlaySurface`. |
| `scripts/online/chat_state.gd` * | 141 | Shared chat save/message/payload helpers; left separate because cross-cutting callers still use it. |

## Diagnostics Helper Scripts

| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/diagnostics/crash_report_runtime.gd` * | 349 | Crash-report runtime owner: pending report file load/delete/store, session id/heartbeat, live session context dictionary, session marker writes, Android diagnostic event file tailing, unclean previous-session synthesis, clipboard formatting, Android event compaction, metadata extraction, and lifecycle verdict helpers. `scripts/main.gd` keeps settings copy UI policy. Explicit preload callers remain after the global class registration deletion. |
...
| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/temporary_events/runtime.gd` * | 957 | Temporary-event runtime owner: scheduler constants, owner-local event module load normalization, event definitions, active/cooldown/next-roll scheduler state, art-review activation, active-event action projection, spawn-level/reward/stamina/seconds math, scheduler/expiry/cooldown/spawn state, event action start/clear/completion bridge, completion state mutation, and runtime-owned save/restore normalization. `scripts/main.gd` keeps compatibility properties/wrappers plus player-visible completion/despawn rendering. |

## Activity Queue Helper Scripts

| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/activity_queue/runtime.gd` * | 309 | Activity queue runtime owner: queue storage, public queue mutations, list normalization, circular next-index math, save/restore bridge, queueability checks, start/stop/process/advance loop, stamina skip using RecoveryModules recovery-full policy, fishing-area target resolution, and running-action queue-key matching. `scripts/main.gd` keeps compatibility properties/wrappers for old scene/test callers. |

## Chat Helper Scripts

| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/online/chat_state.gd` * | 141 | Chat save-state clamping, last-opened message id normalization, message whitespace/censor/max-length sanitation, message id generation, and outbound Firebase payload/update construction. Stream/UI behavior remains in `OnlineRuntime` and chat presentation owners. |
...
| Path | Lines | What lives here |
| --- | ---: | --- |
| `scripts/activity_data/catalog.gd` * | 429 | Live activity catalog owner: `ACTIVITY_DATABASE_PATH`, skill/action collections, database load, owner-local action dictionary load normalization, action lookup rebuild, convergence action id list, and activity display sort. `scripts/main.gd` keeps public aliases/wrappers for existing callers. |
...
| `scripts/test-performance-regressions.ps1` * | 3,446 | Static/runtime regression assertions for performance-sensitive code and UI contracts; hub save-state assertions now target `HubRuntime` owner calls, and achievement/chat/leaderboard/tip/temporary-event state restore, Firebase silent JSON parsing, crash-report formatting, activity-data action normalization, event module loading, profile overlay visibility ownership, leaderboard/profile style ownership, and activity-card style ownership assert extracted helpers. |
| `scripts/test-save-normalization.ps1` * | 2,991 | Save/load normalization regression assertions; hub module/selection/mission direct checks now call the `HubRuntime` owner, and save-file parser/recovery assertions target `SaveRuntime`. |
...
| `scripts/gameplay/buildable_modules.gd` | added/modified | Extracted gameplay helper for buildable action contracts, built-state normalization, build cost, label, XP policy, affordability, and spend helpers; live build transaction moved back to `ActionRuntime`. |
...
| `scripts/tutorial/tip_state.gd` | removed | Folded into `scripts/save_state/normalizers.gd`; tip metadata normalization is save-state normalization, not tutorial UI state. |
...
| `scripts/temporary_events/runtime.gd` | added/modified | Extracted temporary-event runtime owner for constants, definitions, scheduler/state, action projection, reward scaling, event action start/clear/completion attempts, completion mutation, save/restore orchestration, and folded-in active/cooldown normalization. |
...
| `scripts/leaderboard/profile.gd` | moved | Leaderboard profile helper for local name/id/avatar/auth rules now lives under the leaderboard owner folder. |
| `scripts/thieving/state.gd` | added | New extracted thieving helper for trophy and action jail save normalization. |
| Achievement milestone builder helper | added | New extracted achievement milestone builder fed by a live progress context from `scripts/main.gd`. |
| `scripts/save_state/files.gd` | removed | Folded into `scripts/save_state/save_runtime.gd`; save-file JSON load/write, temp/backup promotion, and best-save comparison now live with the save runtime owner. |
...
| `scripts/activity_data/normalizers.gd` | removed | Split back into the two real owners: action load normalization in `scripts/activity_data/catalog.gd`, and event module load normalization in `scripts/temporary_events/runtime.gd`. |
...
| `scripts/achievements/rewards.gd` | removed | Folded into `scripts/achievements/presentation.gd` for art paths and achievement state helpers for target tables/reward formulas. |
...
| `scripts/leaderboard/styles.gd` | removed | Folded into `scripts/leaderboard/presentation.gd` for leaderboard page styles and `scripts/ui/profile_chat_overlay_surface.gd` for profile/avatar form styles. |
...
| `scripts/test-performance-regressions.ps1` | modified | Static assertion updated so skill detail action viewport must clip below the skill info shelf; RegenCircle and FishCircle assertions now target extracted scripts; silent JSON parsing assertion now targets `scripts/online/online_runtime.gd`; crash-report formatting assertions now target `scripts/diagnostics/crash_report_runtime.gd`. Also contains pre-existing save/refactor assertion edits from active worktree. |
...
| `scripts/test-save-normalization.ps1` | modified | Save-file parser/recovery checks now preload `SaveRuntime` instead of the deleted standalone save-file helper. |
...
4. Achievements
   - Current: achievement art paths live in `scripts/achievements/presentation.gd`; target tables, reward formulas, and private milestone dictionary construction live in `scripts/achievements/state.gd`.
   ...
   - Current: `scripts/main.gd` keeps only the retained achievement/toast compatibility points and leaves UI/rendering in place.
   - Next lazy win: keep reward bonus cache invalidation in `scripts/main.gd` until broader stat-cache ownership moves.

5. Progression
   - Current: neighbor medal buff contribution and per-tier math live in `scripts/achievements/state.gd`.
...
7. Leaderboard
   ...
   - Next lazy win: keep `scripts/online/chat_state.gd` and `scripts/leaderboard/profile.gd` separate until their cross-cutting callers can route through clearer owners.

7a. Diagnostics
   - Current: crash-report session marker/pending-file ownership, formatting, Android event compaction, metadata extraction, and lifecycle verdicts live in `scripts/diagnostics/crash_report_runtime.gd`.
...
9. Chat
   - Current: retry timestamp save/restore clamping, opened message-id normalization, message sanitation/censoring, and message id/payload construction live in `scripts/online/chat_state.gd`; chat row timestamp formatting lives in `scripts/ui/profile_chat_overlay_surface.gd`.
   - Next lazy win: keep stream connection in `scripts/online/online_runtime.gd` and row/composer UI in `scripts/ui/profile_chat_overlay_surface.gd` until a full chat runtime boundary can move.
...
11. Activity data loading
   - Current: live activity catalog loading/lookup/sort and action load normalization live in `scripts/activity_data/catalog.gd`; temporary-event module load normalization lives in `scripts/temporary_events/runtime.gd`; fishing area parsing and render-policy catalog ownership live in `scripts/fishing/state.gd`.
...
| Step | Result |
| --- | --- |
| OnlineRuntime global class registration deletion validation | Requested `rg` showed only `scripts/main.gd` local preload/factory/type uses and no remaining global `class_name OnlineRuntime`; scoped `git diff --check`, `.\scripts\test-firebase-leaderboard-runtime-guard.ps1`, `.\run-godot-safe.ps1 --path . --quit-after 1`, and Godot process audit passed. |
...
| `git diff --check -- scripts/main.gd scripts/ui/activity_card_styles.gd` | passed after folding page-switch face and prism connector controls into ActivityCardStyles. |
...
| `git diff --check -- scripts/main.gd plus extracted collapse-arrow and health-gauge controls` | passed after extracting chevron, collapse arrow, and health gauge controls. |
...
| `git diff --check -- scripts/main.gd scripts/diagnostics/crash_report_runtime.gd scripts/test-performance-regressions.ps1` | passed after extracting crash-report diagnostics. |
...
| `git diff --check -- scripts/main.gd scripts/diagnostics/crash_report_runtime.gd scripts/app/lifecycle_runtime.gd scripts/ui/settings_surface.gd scripts/test-crash-report-recovery.ps1 scripts/test-performance-regressions.ps1 docs/refactor-file-map.md docs/codebase-complete-refactor-plan-and-checklist.md` | passed after extracting crash-report runtime; emitted only CRLF conversion warnings. |
...
| `git diff --check -- scripts/main.gd achievement milestone builder helper` | passed after extracting achievement milestone builders. |
| `.\scripts\test-home-achievement-medal-click.ps1` | first run exposed a missing sibling preload in the achievement milestone builder helper, then passed after fixing it; runner emitted existing save-protection/shutdown warnings. |
...
| `git diff --check -- scripts/main.gd scripts/save_state/normalizers.gd scripts/test-performance-regressions.ps1` | passed after extracting tip metadata normalization. |
...
| `git diff --check -- scripts/main.gd scripts/save_state/save_runtime.gd scripts/test-save-normalization.ps1` | passed after folding save-file I/O helpers into `SaveRuntime`. |
...
| `.\scripts\test-performance-regressions.ps1` | first rerun exposed a stale ownership assertion, then passed after pointing it at the temporary-event active-entry normalizer. |
| `git diff --check -- scripts/main.gd scripts/test-performance-regressions.ps1` | passed after extracting temporary-event save-state normalization. |
| Godot process check | no leftover `Godot.exe` processes found after temporary-event validation. |
| `git diff --check -- scripts/main.gd scripts/temporary_events/runtime.gd scripts/save_state/save_runtime.gd scripts/gameplay/action_runtime.gd scripts/fishing/state.gd scripts/test-save-normalization.ps1 scripts/test-module-list-transitions.ps1 scripts/test-performance-regressions.ps1 docs/refactor-file-map.md docs/codebase-complete-refactor-plan-and-checklist.md` | passed after finishing temporary-event runtime ownership; emitted only LF/CRLF conversion warnings. |
...
| `git diff --check -- scripts/main.gd scripts/monetization/ad_bonus.gd scripts/ui/shop_surface.gd scripts/save_state/save_runtime.gd scripts/ui/skill_detail_surface.gd scripts/test-save-normalization.ps1 scripts/test-performance-regressions.ps1 docs/refactor-file-map.md docs/codebase-complete-refactor-plan-and-checklist.md scripts/set-admob-ids.ps1` | passed after moving rewarded-ad/shop bonus runtime ownership; emitted only LF/CRLF conversion warnings. |
...
| `.\scripts\test-performance-regressions.ps1` | first rerun exposed a stale module-utility icon metadata ownership assertion, then passed after moving it to the row builder. |
| `git diff --check -- scripts/main.gd scripts/ui/navigation_shell.gd scripts/test-performance-regressions.ps1` | passed after extracting the module utility row builder. |
| Autoreview | no project/tool `autoreview` runner found; manual diff review plus `git diff --check` found no new issue. |
...
| `.\scripts\test-performance-regressions.ps1` | passed after folding achievement style ownership assertions into `AchievementPresentation`. |
| `git diff --check -- scripts/achievements/presentation.gd scripts/ui/achievement_overlay_surface.gd scripts/ui/achievement_toast_surface.gd scripts/test-performance-regressions.ps1 docs/refactor-file-map.md docs/codebase-complete-refactor-plan-and-checklist.md` | passed after folding achievement styles into presentation. |
| Autoreview | no project/tool `autoreview` runner found; manual diff review plus `git diff --check` found no new issue. |
| Godot process check | no leftover `Godot.exe` processes found after achievement style validation. |
| `git diff --check -- scripts/main.gd scripts/achievements/state.gd achievement milestone builder helper scripts/gameplay/action_runtime.gd scripts/fishing/state.gd scripts/ui/achievement_overlay_surface.gd scripts/ui/skill_detail_surface.gd scripts/save_state/save_runtime.gd scripts/test-performance-regressions.ps1 docs/refactor-file-map.md docs/codebase-complete-refactor-plan-and-checklist.md` | passed after moving achievement live-state/context helpers; emitted only LF-to-CRLF warnings. |
...
| Scoped activity data normalizer owner split validation | Requested `rg` found no live `ActivityDataNormalizers` or deleted-path script/project references; remaining matches were expected owner-local helpers, save-state normalizer references, and historical/deletion doc rows. Scoped `git diff --check` passed with LF-to-CRLF warnings only; `.\scripts\audit-activity-database.ps1`; `.\scripts\test-performance-regressions.ps1`; `.\run-godot-safe.ps1 --path . --quit-after 1`; Godot process audit returned no rows. |
| Godot process check | no leftover `Godot.exe` processes found after temporary-event wrapper-deletion validation. |
| Deleted achievement milestone helper reference search | no matches after folding the one-owner milestone helper into `AchievementState`. |
| Scoped `git diff --check` for achievement state, deleted helper, and docs | passed after folding the one-owner milestone helper into `AchievementState`; emitted only LF-to-CRLF warnings. |
| `.\scripts\test-home-achievement-medal-click.ps1` | passed after folding the one-owner milestone helper; emitted existing save recovery/progress-block and ObjectDB leak-at-exit warnings. |
| `.\scripts\test-performance-regressions.ps1` | passed after folding the one-owner milestone helper. |
| `.\run-godot-safe.ps1 --path . --quit-after 1` | passed after folding the one-owner milestone helper. |
| Godot process check | no leftover `Godot.exe` processes found after achievement milestone fold-in validation. |
