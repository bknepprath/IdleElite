...
static func display_name_for_save(host, display_max_chars: int) -> String:
	return sanitize_display_name(str(host.get("leaderboard_display_name")), display_max_chars)
...
static func profile_metadata_for_save(host, guest_prefix: String, display_max_chars: int, key_max_chars: int) -> Dictionary:
	return metadata_for_save(
		str(host.get("leaderboard_display_name")),
		str(host.get("leaderboard_name_key")),
		bool(host.get("leaderboard_profile_claimed")),
		bool(host.get("leaderboard_name_claim_verified")),
		guest_prefix,
		display_max_chars,
		key_max_chars
	)
...
static func restore_profile_metadata_from_save(host, data: Dictionary, guest_prefix: String, display_max_chars: int, key_max_chars: int, avatar_count: int) -> void:
	var metadata := restored_metadata(
		...
		str(host.get("leaderboard_display_name")),
		str(host.get("leaderboard_name_key")),
		int(host.get("leaderboard_avatar_index")),
		str(host.get("leaderboard_player_id")),
		guest_prefix,
		display_max_chars,
		key_max_chars,
		avatar_count
	)
	host.set("leaderboard_display_name", str(metadata.get("display_name", host.get("leaderboard_display_name"))))
	host.set("leaderboard_name_key", str(metadata.get("name_key", "")))
	host.set("leaderboard_profile_claimed", bool(metadata.get("profile_claimed", false)))
	host.set("leaderboard_name_claim_verified", bool(metadata.get("name_claim_verified", false)))
	host.set("leaderboard_avatar_index", int(metadata.get("avatar_index", host.get("leaderboard_avatar_index"))))
	host.set("leaderboard_player_id", str(metadata.get("player_id", "")))
	if str(host.get("leaderboard_player_id")).is_empty():
		host.set("leaderboard_player_id", make_player_id())


static func avatar_index_for_save(host, avatar_count: int) -> int:
	return valid_avatar_index(int(host.get("leaderboard_avatar_index")), avatar_count)


static func player_id_for_save(host) -> String:
	return sanitize_player_id(str(host.get("leaderboard_player_id")))
...
static func auth_refresh_token_for_save(host) -> String:
	return refresh_token_for_save(str(host.get("leaderboard_auth_refresh_token")))


static func auth_provider_for_save_host(host) -> String:
	return auth_provider_for_save(str(host.get("leaderboard_auth_provider")))


static func restore_auth_metadata_from_save(host, data: Dictionary) -> void:
	host.set("leaderboard_auth_id_token", "")
	host.set("leaderboard_auth_refresh_token", refresh_token_for_save(str(data.get("leaderboard_auth_refresh_token", ""))))
	host.set("leaderboard_auth_expires_unix", 0)
	host.set("leaderboard_auth_retry_after_unix", maxi(0, int(data.get("leaderboard_auth_retry_after_unix", 0))))
	host.set("leaderboard_auth_provider", auth_provider_for_save(str(data.get("leaderboard_auth_provider", "")).strip_edges()))


static func profile_claim_valid(host, guest_prefix: String, display_max_chars: int, key_max_chars: int) -> bool:
	if not bool(host.get("leaderboard_profile_claimed")) or not bool(host.get("leaderboard_name_claim_verified")):
		return false
	if str(host.get("leaderboard_name_key")).is_empty():
		host.set("leaderboard_name_key", name_key(str(host.get("leaderboard_display_name")), display_max_chars, key_max_chars))
	return not str(host.get("leaderboard_name_key")).is_empty() and not is_guest_display_name(str(host.get("leaderboard_display_name")), guest_prefix, display_max_chars)
