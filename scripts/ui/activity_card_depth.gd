...
func _draw_wide_u_back_plate(rect: Rect2) -> void:
	var points := _wide_u_back_outline_points(rect, 0.0)
	if points.size() < 4:
		return
...
func _draw_wide_u_back_outline(rect: Rect2, color: Color, width: float) -> void:
	var points := _wide_u_back_outline_points(rect, width)
	if points.size() >= 2:
		draw_polyline(points, color, width, true)

func _wide_u_back_outline_points(rect: Rect2, width: float) -> PackedVector2Array:
	var half := width * 0.5
	var left := rect.position.x + half
	var right := rect.end.x - half
	var top := rect.position.y + half
	var bottom := rect.end.y - half
	...
	_append_arc_points(points, Vector2(right - r, top + r), r, -PI * 0.5, 0.0, OUTLINE_ARC_SEGMENTS)
	_append_unique_point(points, Vector2(right, side_y - r))
	_append_arc_points(points, Vector2(right - r, side_y - r), r, 0.0, PI * 0.5, OUTLINE_ARC_SEGMENTS)
	_append_unique_point(points, Vector2(curve_right, side_y))
	for index in range(1, 35):
		var t := float(index) / 34.0
		var curve_weight := sin(t * PI)
		_append_unique_point(points, Vector2(lerpf(curve_right, curve_left, t), lerpf(side_y, bottom, curve_weight * curve_weight)))
	_append_unique_point(points, Vector2(curve_left, side_y))
	_append_unique_point(points, Vector2(left + r, side_y))
	_append_arc_points(points, Vector2(left + r, side_y - r), r, PI * 0.5, PI, OUTLINE_ARC_SEGMENTS)
	_append_unique_point(points, Vector2(left, top + r))
	_append_arc_points(points, Vector2(left + r, top + r), r, PI, PI * 1.5, OUTLINE_ARC_SEGMENTS)
	if not points.is_empty():
		_append_unique_point(points, points[0])
	return points
