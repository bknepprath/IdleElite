...
func _set_temporary_event_entry_height_safe(height: float, host_id: int) -> void:
	var entry_host: Control = host._app_lifecycle_runtime().valid_control_ref(instance_from_id(host_id))
...
func _finish_temporary_event_entry_reveal(host_id: int, fade_target_id: int, target_height: float) -> void:
	...
	var fade_target: Control = host._app_lifecycle_runtime().valid_control_ref(instance_from_id(fade_target_id))
...
func _play_temporary_event_entry_reveal(entry_host: Control, fade_target: Control, target_height: float) -> void:
	if entry_host == null or not is_instance_valid(entry_host):
		return
	host._app_lifecycle_runtime()._kill_meta_tween(entry_host, "temporary_event_entry_tween")
...
func _detail_lazy_insert_animated_temporary_event_entry(
	...
	host._skill_detail_surface()._detail_lazy_create_slot_for_entry(stack, skill_id, event_entry, content_width, actions_width)
	var entry_host: Control = host._app_lifecycle_runtime().valid_control_ref(event_entry.get("stack_host"))
	if entry_host == null or not is_instance_valid(entry_host):
		return false
	var insert_index: int = host._skill_detail_surface()._detail_lazy_stack_insert_index_for_plan_index(stack, plan_index)
	stack.move_child(entry_host, clampi(insert_index, 0, maxi(0, stack.get_child_count() - 1)))
	var target_height := maxf(1.0, float(event_entry.get("height", ActivityCardStyles.root_height(false, host.ACTION_CARD_HEIGHT, host.ACTION_CARD_EXPANDED_HEIGHT, host.ACTION_CARD_3D_DEPTH_OFFSET.y))))
	entry_host.clip_contents = true
	_set_temporary_event_entry_height(entry_host, 0.0)
	if not host._skill_detail_surface()._detail_lazy_mount_item(event_entry, skill_id, content_width, actions_width, false):
		host._skill_detail_surface()._detail_lazy_remove_unmounted_inserted_host(event_entry)
		return false
	entry_host = host._app_lifecycle_runtime().valid_control_ref(event_entry.get("stack_host"))
	if entry_host == null or not is_instance_valid(entry_host):
		return false
	var fade_target: Control = host._skill_detail_surface()._detail_lazy_primary_child_control(entry_host)
...
func _animate_temporary_event_entry_if_visible(event_def: Dictionary, event_id: String) -> bool:
	...
	if host.current_screen != "skill" or host.selected_skill_id != skill_id:
		return false
	if host._navigation_shell().screen_render_in_progress or host.boot_detail_render_in_progress or host._skill_swipe_activity_surface()._skill_swipe_navigation_blocks_detail_refresh():
		return false
	...
	if host._skill_detail_surface().detail_actions_scroll == null or not is_instance_valid(host._skill_detail_surface().detail_actions_scroll):
		return false
	var stack: VBoxContainer = host._skill_detail_surface().detail_lazy_stack
	if stack == null or not is_instance_valid(stack):
		stack = host._detail_actions_stack() as VBoxContainer
	if stack == null or not is_instance_valid(stack):
		return false
	if host._skill_detail_surface().detail_lazy_plan.is_empty() or not host._skill_swipe_activity_surface()._skill_detail_stack_has_visible_modules(stack):
		return false
	if host._skill_detail_surface().detail_rendered_action_ids.has(event_id) or host._skill_detail_surface().detail_action_card_nodes.has(event_id):
		return true
	...
	if event_index < 0:
		return false
	var old_entries_by_track_id: Dictionary = host._skill_detail_surface()._detail_lazy_runtime_entries_by_track_id(host._skill_detail_surface().detail_lazy_plan)
	for raw_new_entry in new_plan:
		...
		if track_id == event_id or not old_entries_by_track_id.has(track_id):
			continue
		host._skill_detail_surface()._detail_lazy_copy_runtime_entry_state(new_entry, old_entries_by_track_id[track_id] as Dictionary)
	...
	var layout_snapshot: Dictionary = host._skill_detail_surface()._capture_detail_module_layout_snapshot()
	if not _detail_lazy_insert_animated_temporary_event_entry(stack, skill_id, event_entry, event_index, content_width, actions_width):
		return false
	host._skill_detail_surface()._detail_lazy_reorder_existing_hosts_for_plan(stack, new_plan, event_id)
	host._skill_detail_surface().detail_lazy_stack = stack
	host._skill_detail_surface().detail_lazy_plan = new_plan
	host._skill_detail_surface().detail_rendered_action_ids = new_signature
	host._skill_detail_surface().detail_lazy_last_scroll = host._skill_detail_surface()._detail_lazy_scroll_y()
	host._skill_detail_surface()._hold_skill_detail_layout_refresh(
		TEMPORARY_EVENT_ENTRY_EXPAND_SECONDS
...
func _play_temporary_event_completion_exit(skill_id: String, action_id: String, xp_reward: int) -> void:
	...
	host._skill_detail_surface()._hold_skill_detail_layout_refresh(2.55)
	root.mouse_filter = Control.MOUSE_FILTER_IGNORE
	root.clip_contents = true
	var restore_scroll: int = host._skill_detail_surface().detail_actions_scroll.scroll_vertical if host._skill_detail_surface().detail_actions_scroll != null else -1
	...
	if tint_target != null:
		...
		host._app_lifecycle_runtime().set_canvas_item_modulate_if_changed(tint_target, Color("#9cff9e"))
	if art_panel != null:
...
func _refresh_skill_detail_after_temporary_event_despawn(skill_id: String, restore_scroll := -1) -> void:
	if host.current_screen != "skill" or host.selected_skill_id != skill_id:
		...
		return
	var event_refresh_scroll: int = restore_scroll if restore_scroll >= 0 else (host._skill_detail_surface().detail_actions_scroll.scroll_vertical if host._skill_detail_surface().detail_actions_scroll != null else -1)
	await host._skill_detail_surface()._refresh_visible_skill_detail_action_list(event_refresh_scroll, skill_id, true)
