...
func _build_achievement_toast_layer() -> void:
	...
	achievement_toast_layer.layer = host.ACHIEVEMENT_TOAST_CANVAS_LAYER
...
func show_pending_completed_toasts() -> void:
	while host.boot_warmup_active:
		await host.get_tree().process_frame
	while host.pending_post_load_saved_at >= 0 or host.boot_post_load_simulation_scheduled:
		await host.get_tree().process_frame
	await host.get_tree().process_frame
	while host.offline_summary_overlay != null and host.offline_summary_overlay.visible:
		await host.get_tree().process_frame
...
func _achievement_toast_queue_blocked() -> bool:
	if host.boot_warmup_active:
		return true
	if host.pending_post_load_saved_at >= 0 or host.boot_post_load_simulation_scheduled:
		return true
	return host.offline_summary_overlay != null and host.offline_summary_overlay.visible
...
func _achievement_toast_queue_badge_for_toast(toast: Control) -> Control:
	...
	var badge: Control = host._valid_control_ref(instance_from_id(badge_id))
...
func _achievement_toast_queue_badge() -> PanelContainer:
	...
	badge.custom_minimum_size = host.ACHIEVEMENT_TOAST_QUEUE_BADGE_SIZE
	badge.size = host.ACHIEVEMENT_TOAST_QUEUE_BADGE_SIZE
	badge.mouse_filter = Control.MOUSE_FILTER_IGNORE
	badge.z_index = 12
	badge.add_theme_stylebox_override("panel", host.AchievementStyles.toast_queue_badge())
...
func _update_achievement_toast_queue_badge(toast: Control, badge: Control, queued_count: int) -> void:
	...
	badge.position = Vector2(
		toast.size.x - (host.ACHIEVEMENT_TOAST_QUEUE_BADGE_SIZE.x + host.ACHIEVEMENT_TOAST_QUEUE_BADGE_OFFSET.x) * fitted_scale,
		host.ACHIEVEMENT_TOAST_QUEUE_BADGE_OFFSET.y * fitted_scale
	)
	var label_id := int(badge.get_meta("achievement_toast_queue_badge_label_id", 0))
	var label: Label = host._valid_label_ref(instance_from_id(label_id))
	if label != null:
...
func _present_achievement_toast(achievement: Dictionary) -> void:
	...
	var fitted_scale: float = host._fit_scale_to_canvas(host.ACHIEVEMENT_TOAST_SIZE, host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN)
	var presentation_size: Vector2 = host.ACHIEVEMENT_TOAST_SIZE * fitted_scale
	...
	card.custom_minimum_size = host.ACHIEVEMENT_TOAST_SIZE
	card.size = host.ACHIEVEMENT_TOAST_SIZE
	...
	var bottom_inset := minf(host.BOTTOM_NAV_HEIGHT, canvas_size.y * 0.22)
	var target_position := Vector2(
		(canvas_size.x - presentation_size.x) * 0.5,
		canvas_size.y - bottom_inset - presentation_size.y - host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.y - float(toast_index) * (presentation_size.y + host.ACHIEVEMENT_TOAST_GAP * fitted_scale)
	)
	target_position.x = clampf(
		target_position.x,
		host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.x,
		maxf(host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.x, canvas_size.x - presentation_size.x - host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.x)
	)
	target_position.y = clampf(
		target_position.y,
		host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.y,
		maxf(host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.y, canvas_size.y - presentation_size.y - host.ACHIEVEMENT_TOAST_VIEWPORT_MARGIN.y)
	)
	banner.position = target_position + Vector2(0, 90.0 * fitted_scale)
	host._set_canvas_item_modulate_if_changed(banner, Color(1, 1, 1, 0))
	...
	tween.chain().tween_interval(host.ACHIEVEMENT_TOAST_EXIT_DELAY)
...
func _on_achievement_toast_gui_input_bound(event: InputEvent, banner_id: int, exit_offset: Vector2) -> void:
	var banner: Control = host._valid_control_ref(instance_from_id(banner_id))
...
func _route_achievement_toast_input(event: InputEvent) -> bool:
	...
	for i in range(achievement_toasts.size() - 1, -1, -1):
		...
		if not _achievement_toast_accepts_dismiss_event(event, toast):
			continue
		host._input_routing_shell()._block_background_input_briefly()
		_dismiss_achievement_toast(toast, host._meta_vector2(toast, "achievement_exit_offset", Vector2(0, 110.0)), false)
		return true
	return false
...
func _dismiss_achievement_toast(banner: Control, exit_offset: Vector2, automatic := false) -> void:
	...
	banner.set_meta("achievement_tween", tween)
	tween.set_parallel(true)
	var exit_seconds: float = host.ACHIEVEMENT_TOAST_AUTO_EXIT_SECONDS if automatic else host.ACHIEVEMENT_TOAST_TAP_EXIT_SECONDS
...
func _transition_achievement_toast_to_next_card(banner: Control, exit_offset: Vector2, automatic := false) -> void:
	...
	card.size = host.ACHIEVEMENT_TOAST_SIZE
	card.scale = Vector2(fitted_scale, fitted_scale)
	host._set_canvas_item_modulate_if_changed(card, Color(1, 1, 1, 0.92))
	banner.add_child(card)
	banner.move_child(card, 0)
	banner.set_meta("achievement_toast_card_id", card.get_instance_id())
	_sync_achievement_toast_queue_badges()
	banner.scale = Vector2(0.94, 0.94)
	host._set_canvas_item_alpha_if_changed(banner, 1.0)
	var banner_id := banner.get_instance_id()
	var tween: Tween = host.create_tween()
	banner.set_meta("achievement_tween", tween)
	tween.set_parallel(true)
	tween.tween_property(banner, "scale", Vector2(1.06, 1.06), 0.13).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(card, "modulate:a", 1.0, 0.08)
	tween.chain().tween_property(banner, "scale", Vector2.ONE, 0.18).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
...
func _achievement_toast_card_for_banner(banner: Control) -> Control:
	...
	var card: Control = host._valid_control_ref(instance_from_id(card_id))
...
func _kill_achievement_toast_tween(banner: Control) -> void:
	if not _achievement_toast_live(banner):
		return
	host._kill_meta_tween(banner, "achievement_tween")
...
func card(achievement: Dictionary) -> Control:
	...
	var text_accent := _text_accent(achievement, accent)
	var card := PanelContainer.new()
	card.clip_contents = true
	card.mouse_filter = Control.MOUSE_FILTER_IGNORE
	card.add_theme_stylebox_override("panel", host.AchievementStyles.card(Color("#fffdf8") if completed else Color("#fff6e1"), 34, 28, Callable(host, "_surface_style")))
	...
	art_holder.add_child(host._achievement_overlay_surface()._achievement_art(achievement))
	...
	var eyebrow: Label = host._label("ACHIEVEMENT UNLOCKED", host.MIN_MOBILE_BODY_FONT_SIZE, text_accent, HORIZONTAL_ALIGNMENT_LEFT)
	eyebrow.mouse_filter = Control.MOUSE_FILTER_IGNORE
	copy.add_child(eyebrow)

	var title_label: Label = host._label(str(achievement.get("title", "Achievement")), 76, host.COLOR_INK, HORIZONTAL_ALIGNMENT_LEFT)
	...
	var subtitle_label: Label = host._label(str(achievement.get("subtitle", "")), 48, host.COLOR_INK, HORIZONTAL_ALIGNMENT_LEFT)
	...
	var reward_label: Label = host._label(_reward_text(achievement), 50, host.COLOR_INK, HORIZONTAL_ALIGNMENT_LEFT)
...
func _text_accent(achievement: Dictionary, accent: Color) -> Color:
