...
func _init() -> void:
	...
	online.leaderboard_config_loaded = true
	online.leaderboard_config_database_url = "https://idle-elite-default-rtdb.firebaseio.com/"
	...
	online.leaderboard_config_database_url = "https://idle-elite.example.com"
	_expect(online.call("_leaderboard_firebase_base_url") == "", "Runtime base URL getter should fail closed for malformed hosts.")
	online.leaderboard_config_database_url = "https://your-project-id-default-rtdb.firebaseio.com"
	_expect(online.call("_leaderboard_firebase_base_url") == "", "Runtime base URL getter should fail closed for placeholder hosts.")
	online.leaderboard_config_web_api_key = "AIzaSyValidationOnlyNotARealFirebaseKey123456"
	...
	online.leaderboard_config_web_api_key = "too-short"
	_expect(online.call("_leaderboard_firebase_api_key") == "", "Runtime API key getter should reject short keys.")
	online.leaderboard_config_web_api_key = "AIzaSy Validation Only Key"
	_expect(online.call("_leaderboard_firebase_api_key") == "", "Runtime API key getter should reject whitespace-damaged keys.")
	online.leaderboard_config_database_url = ""
	online.leaderboard_config_web_api_key = ""
	online.leaderboard_auth_in_flight = false
	game.leaderboard_state.fetch_in_flight = false
	online.leaderboard_submit_in_flight = false
	online.cloud_save_fetch_in_flight = false
	online.cloud_save_upload_in_flight = false
	game.current_screen = "leaderboard"
	online.fetch_leaderboard_category(LeaderboardState.CATEGORY_TOTAL_LEVEL)
	...
	_expect(game.leaderboard_state.status_message == "Online services are not connected yet.", "Absent config should produce the fail-closed leaderboard status.")
	...
	online.leaderboard_config_database_url = "https://idle-elite-default-rtdb.firebaseio.com/"
	online.leaderboard_config_web_api_key = "AIzaSyValidationOnlyNotARealFirebaseKey123456"
	online.google_auth_web_client_id = ""
	...
	_expect(online.google_auth_status_message == "Google sign-in was cancelled.", "Empty Google failure should read as a cancellation.")
	...
	_expect(online.google_auth_status_message == "Google sign-in was cancelled.", "Native cancellation text should be player-friendly.")
	online.call("_on_google_sign_in_failed", "No credentials available")
	_expect(online.google_auth_status_message == "No Google account was selected. Try Connect Google again.", "Missing credential text should tell the player what to do next.")
...
func _expect_leaderboard_requests_idle(game: Node, context: String) -> void:
	var online = game._online_runtime()
	_expect(not online.leaderboard_auth_in_flight, "%s should not start Firebase Auth." % context)
	_expect(not game.leaderboard_state.fetch_in_flight, "%s should not start a leaderboard read." % context)
	_expect(not online.leaderboard_submit_in_flight, "%s should not start a leaderboard write." % context)
	_expect(not online.cloud_save_fetch_in_flight, "%s should not start a cloud save read." % context)
	_expect(not online.cloud_save_upload_in_flight, "%s should not start a cloud save write." % context)
