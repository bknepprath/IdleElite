...
func _run() -> void:
	...
	scene.call("_action_runtime").call("_process_action", 9.0)
	_expect(int((scene.skills["fight"] as Dictionary).get("xp", 0)) == starting_xp + 1, "recovery action should grant its tiny XP reward")
	_expect(absf(float(scene.call("_stamina_value", "fight")) - 4.0) < 0.001, "recovery action should restore 3 Fighting stamina")
	...
	var lowest_result := RecoveryModules.apply("thieving", lowest_action, scene.skill_defs, scene.stamina, Callable(scene, "_stamina_value"), Callable(scene, "_max_stamina"), Callable(scene, "_restore_action_stamina"))
