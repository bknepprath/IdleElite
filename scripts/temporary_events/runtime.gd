...
func _load_event_module_definitions(data: Dictionary) -> void:
	...
	for raw_event in loaded_events:
		if typeof(raw_event) != TYPE_DICTIONARY:
			continue
		var event_def := _event_module_for_load(raw_event as Dictionary, definition_order)
...
static func _event_module_for_load(source: Dictionary, definition_order: int) -> Dictionary:
...
func _event_module_sort_less(left: Variant, right: Variant) -> bool:
	...
	var left_sort = host.activity_data_catalog.activity_action_display_sort_level(left_event)
	var right_sort = host.activity_data_catalog.activity_action_display_sort_level(right_event)
...
func _temporary_event_highest_unlocked_page_level(page: String) -> int:
	if page.is_empty() or not host.actions_by_skill.has(page):
		return 0
	var page_level = SkillState.host_skill_level(host, page) if host.skills.has(page) else 0
	var highest := 0
	for raw_action in host.actions_by_skill.get(page, []):
		...
		if action.is_empty() or host._is_event_action(action):
			continue
		if not host._activity_unlock_runtime()._is_action_unlocked(page, action):
			continue
		var display_level = host.activity_data_catalog.activity_action_display_sort_level(action)
...
func _temporary_event_spawn_level_for_page(page: String, event_id: String, roll_unix: int) -> int:
	...
	rng.seed = int(abs(hash("%s:%s:%s:%s:%s" % [
		...
		event_id,
		str(highest_level),
		str(SkillState.global_level(host.skills))
	])))
...
func _temporary_event_reference_action_for_level(page: String, spawn_level: int) -> Dictionary:
	...
	for raw_action in host.actions_by_skill.get(page, []):
		if typeof(raw_action) != TYPE_DICTIONARY:
			continue
		var action := raw_action as Dictionary
		if action.is_empty() or host._is_event_action(action) or host._passive_modules_runtime().is_passive_action(action) or host._convergence_runtime()._is_convergence_action(action):
			continue
		var action_level = host.activity_data_catalog.activity_action_display_sort_level(action)
...
func _temporary_event_distribute_reward_total(event_def: Dictionary, target_total: int) -> Dictionary:
	...
	var page := str(event_def.get("page", "")).strip_edges()
	return host._action_runtime()._distribute_xp_reward_map_to_total(template, page, target_total)
...
func _temporary_event_reward_map_for_level(event_def: Dictionary, spawn_level: int) -> Dictionary:
	...
	var reference_rewards = host._action_runtime()._base_xp_reward_map(reference_action, page) if not reference_action.is_empty() else {}
	var reference_total = host._action_runtime()._reward_map_total(reference_rewards)
...
func _temporary_event_log_reward_mat_id() -> String:
	...
	for raw_action in host.actions_by_skill.get("woodcutting", []):
		...
		if not host._activity_unlock_runtime()._is_action_unlocked("woodcutting", woodcutting_action):
			continue
...
func _temporary_event_action_for_entry(event_def: Dictionary, event_entry: Dictionary) -> Dictionary:
	...
	event_action["xp"] = maxi(1, int(reward_map.get(page, host._action_runtime()._reward_map_total(reward_map))))
...
func _active_event_actions_for_skill(skill_id: String) -> Array:
	...
	if active_actions.size() > 1:
		active_actions.sort_custom(func(left, right): return host.activity_data_catalog.activity_action_display_sort_less(left, right))
	return active_actions
...
func reset() -> void:
	...
	temporary_event_scheduler_elapsed = 0.0
	event_running_skill_id = ""
	event_running_action_id = ""
	event_action_progress = 0.0
...
func _start_temporary_event_action(skill_id: String, action_id: String, action: Dictionary, select_page := true) -> bool:
	...
	if event_running_skill_id == skill_id and event_running_action_id == action_id:
		host._reward_feedback_surface()._set_result("Event already underway: %s." % str(action.get("name", "Event")))
	...
	if not host._action_runtime()._auto_eat_fish_for_action(skill_id, stamina_cost, host._skill_detail_surface().detail_regen_circle, true):
		host._reward_feedback_surface()._set_result(_event_needs_stamina_text(skill_id, action))
		host._reward_feedback_surface()._float_event_need_stamina_feedback(action_key, stamina_cost)
		return false
	if select_page:
		host.selected_skill_id = skill_id
	event_running_skill_id = skill_id
	event_running_action_id = action_id
	event_action_progress = 0.0
	...
	host._reward_feedback_surface()._set_result("Event started: %s." % str(action.get("name", "Event")))
...
func _stop_temporary_event_action_with_feedback(skill_id: String, action_id: String, result_text: String, popup_text: String, color: Color) -> void:
	...
	host._reward_feedback_surface()._set_result(result_text)
...
func _complete_temporary_event_action_attempt(skill_id: String, action_id: String, action: Dictionary, reward_key: String, cost: float, bonus_snapshot_before: Dictionary, clear_running_action_on_success := true) -> void:
	...
	if success:
		var xp_reward_map = host._action_runtime()._completion_xp_reward_map(action, skill_id, false, false, false, false)
		old_reward_skill_levels = host._action_runtime()._skill_levels_for_reward_map(skill_id, xp_reward_map)
		...
		host.last_result = "Event complete: %s" % host._action_runtime()._xp_reward_result_sentence(xp_reward_map, skill_id, str(action.get("name", "Event")))
		if log_reward > 0:
...
func _restore_temporary_events_from_save(value: Variant) -> void:
	if typeof(value) != TYPE_DICTIONARY:
		temporary_event_active = {}
		temporary_event_cooldowns = {}
		temporary_event_next_roll_unix = 0
		return
...
func _temporary_event_active_entry_from_save(event_id_hint: String, value: Variant) -> Dictionary:
	...
	var definition := _event_module_def(event_id)
	if definition.is_empty():
		return {}
	if not bool(source.get("completed", source.get("completion_state", false))) and not _temporary_event_page_level_eligible(definition):
		return {}
	...
	var spawn_level := _temporary_event_spawn_level_from_entry(source, definition)
	return {
		"id": event_id,
