...
Assert-True ($main -match '_leaderboard_firebase_url\("player_write_gates/%s" % leaderboard_player_id') "Writes must claim the shared per-player 15-minute write gate before score rows."
Assert-True ($main -match 'leaderboard_pending_score_updates') "Score rows must be staged while the shared write gate is claimed."
...
Assert-True ($main -match 'FIREBASE_AUTH_SIGN_IN_WITH_IDP_URL') "Google account linking must exchange native Google ID tokens through Firebase Auth REST."
...
Assert-True ($main -match '"leaderboard_auth_retry_after_unix": _leaderboard_auth_retry_after_unix_for_save\(\)') "Leaderboard auth retry deadlines must be saved across relaunches."
...
Assert-True ($main -match '"chat_last_send_unix": _chat_last_send_unix_for_save\(\)') "Chat send cooldown must be saved across relaunches."
