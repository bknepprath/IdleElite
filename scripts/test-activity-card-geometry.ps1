...
func _run() -> void:
	...
	MainScript.ActivityCardStyles.configure_activity_card_title(activity_title)
	_expect(activity_title.get_theme_font_size("font_size") == 120, "Activity-card titles should render at least 60 physical pixels when the 2160-wide canvas is shown at 1080 width.")
	_expect(activity_title.autowrap_mode == TextServer.AUTOWRAP_OFF and activity_title.max_lines_visible == 1, "Activity-card titles should stay on one line.")
	_expect(activity_title.text_overrun_behavior == TextServer.OVERRUN_NO_TRIMMING and not activity_title.clip_text, "Activity-card titles should use the full-width title lane without ellipsis or clipping.")
	activity_title.free()
	_expect(MainScript.MIN_MOBILE_BODY_FONT_SIZE == 96, "Body text should render at least 48 physical pixels when the 2160-wide canvas is shown at 1080 width.")
	_expect(MainScript.MIN_MOBILE_HELP_FONT_SIZE == 104, "Help and status text should render at least 52 physical pixels when the 2160-wide canvas is shown at 1080 width.")
	_expect(MainScript.MIN_MOBILE_INFO_TITLE_FONT_SIZE == 120, "Information titles should render at least 60 physical pixels when the 2160-wide canvas is shown at 1080 width.")
