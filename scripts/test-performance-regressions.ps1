...
Assert-True ($crashReportClipboardText -match '_crash_report_android_metadata_summary' -and $crashReportClipboardText -match '_append_crash_report_event_summary\(lines, events, 8\)') "Crash report clipboard text should keep metadata once and use a compact Android event tail."
$previousSessionCrashSummary = Get-FunctionBody -Text $main -Name "_append_previous_session_crash_summary"
...
$compactAndroidDiagnosticEvent = Get-FunctionBody -Text $main -Name "_compact_android_diagnostic_event"
Assert-True ($compactAndroidDiagnosticEvent -match '_android_diagnostic_event_name' -and $compactAndroidDiagnosticEvent -match 'millis_separator') "Android diagnostic events should be compacted to timestamp plus event name."
$previousAndroidLifecycleWasClean = Get-FunctionBody -Text $main -Name "_previous_android_lifecycle_was_clean"
